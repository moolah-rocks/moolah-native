...
- **Backend:** `BackendProvider` is the injection point. `CloudKitBackend` is the production backend. `TestBackend` (CloudKitBackend with an in-memory SwiftData container) is used in tests; `PreviewBackend` is used in SwiftUI previews.
...
- **Contract Tests:** Every repository protocol has a contract test suite in `MoolahTests/Domain/`. Tests run against `CloudKitBackend` with in-memory SwiftData.
