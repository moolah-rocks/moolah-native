...
extension ProfileSession {
  ...
  /// crypto token store, search service, CoinGecko catalog, the CryptoCompare
  /// and Binance token caches, and the token resolution client. Populated for
  /// CloudKit profiles; nil fields indicate a degraded state (e.g. catalog or
  /// cache init failure).
  ///
  /// `catalogRefreshTask` carries the once-per-session CoinGecko
  /// `refreshIfStale()` background task; `cacheRefreshTasks` carries the
  /// once-per-session CryptoCompare/Binance `refreshIfStale()` tasks. Both are
  /// stored on `ProfileSession` so they can be cancelled on teardown. `nil`
  /// (or an empty array) when the corresponding construction failed.
  ///
  /// `cryptoCompareCache` / `binanceCache` are exposed so PR2's reconciliation
  /// can reach the same warm cache instances the resolver reads.
  ...
  /// init failure).
  ///
  /// CloudKit profiles also build a `SQLiteCoinGeckoCatalog`, a
  /// `CryptoCompareTokenCache`, and a `BinanceTokenCache`, firing each one's
  /// `refreshIfStale()` once per session on a background task so the on-disk
  /// snapshots honour the 24 h max-age + ETag guards without blocking session
  /// init. A construction failure (e.g. a SQLite file can't be opened) is
  /// logged and the corresponding handle is left `nil` — search/resolution
  /// degrade to the live-download / registry / Yahoo paths only.
  ///
  ...
  static func makeRegistryWiring(
    ...
    return RegistryWiring(
      ...
      cryptoCompareCache: wiring.cryptoCompareCache,
  ...
  /// Builds the catalog, the CryptoCompare/Binance caches, and the resolution
  /// client (with its background refresh tasks). Under UI testing it returns
  /// the seed's override catalog + resolver and no live caches.
  @MainActor
  private static func makeResolutionWiring(
    ...
    let made = makeCoinGeckoCatalog(
      coinGeckoApiKeyProvider: coinGeckoApiKeyProvider, networking: networking)
    // Build the CryptoCompare / Binance caches alongside the CoinGecko
    // catalog, each firing its own once-per-session background
    // `refreshIfStale()`. The resolver consults them in place of an ad-hoc
    // full coin-list / exchange-info download; a nil cache (open failure)
    // degrades the resolver to its live-download path.
    let madeCryptoCompare = makeCryptoCompareCache(networking: networking)
    let madeBinance = makeBinanceCache(networking: networking)
    let cacheRefreshTasks = [madeCryptoCompare.refreshTask, madeBinance.refreshTask]
      .compactMap { $0 }
    ...
    let resolutionClient = CompositeTokenResolutionClient(
      ...
      cryptoCompareLookup: madeCryptoCompare.cache,
      binanceLookup: madeBinance.cache)
    return ResolutionWiring(
      catalog: made.catalog,
      cryptoCompareCache: madeCryptoCompare.cache,
