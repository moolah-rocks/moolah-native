  ...
  /// system can hand the route off to a peer device. Attached inside the
  /// `activeSession` branch so the `AccountStore` / `EarmarkStore`
  /// environment values (read for title lookup) are guaranteed to be
  /// present.
