...
/// in the three genuine crypto differences (the 4-provider fetch + coalescing,
/// the `.USD` quote denomination, and the confirmed first-trade floor).
extension CryptoPriceService: PriceSeriesOrchestrating {
  ...
  func fetchAndMerge(instrumentKey: String, window: ClosedRange<Date>) async throws {
    guard let context = pendingFetchContext[instrumentKey] else { return }
    try await fetchWindowCoalesced(
      instrument: context.instrument, mapping: context.mapping, fetchInterval: window)
  }

  /// Plug 2 — crypto prices are always stored in USD (set at merge time).
  func quote(for instrumentKey: String) -> Instrument? { .USD }

  /// Plug 3a — the confirmed first-trade floor (`YYYY-MM-DD`), if known.
  func firstTradeFloor(for instrumentKey: String) -> String? {
    caches[instrumentKey]?.firstTradedOn
  }

  /// Plug 3b — confirm+persist the first-trade floor on a clean backward
