// Shared/CryptoImport/LiveAlchemyClient.swift
import Foundation
import OSLog
import os
...
extension LiveAlchemyClient: ChainDataClient {
  /// Alchemy's `alchemy_getAssetTransfers` has no caller-supplied head
  /// concept — every request resolves `toBlock: "latest"` itself — so
  /// this client is not block-range scannable. `nil` tells a windowed
  /// caller (a future direct-RPC-only capability) to skip Alchemy for
  /// window-bound scanning.
