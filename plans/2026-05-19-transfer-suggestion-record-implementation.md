...
- Modify: `Domain/Models/DataFormatVersion.swift` + `Domain/Models/DataFormatVersion.swift` docstring (L31–36) per the `modifying-cloudkit-schema` rubric
...
- [ ] **Step 5: DataFormatVersion** — follow the `modifying-cloudkit-schema` rubric (invoked in Task 7). Default per design doc: since the prior bump to `3` is itself unshipped and the record types/fields are not in the prod baseline, the runbook's expected outcome is to KEEP `current` and revise the `History` doc entry for version 3 to describe the final shape (record type, no denormalised columns). The general-rule alternative (bump `3 → 4` with a new `History` entry, per the additivity rubric for a forward-incompatible change) applies ONLY if Task 7's baseline check found these in production. Apply whichever the runbook + `@database-schema-review` direct; update `MoolahTests/Domain/DataFormatVersionTests.swift` only if an exact pin is added (today it asserts `>= 1`, so no change), and verify `MoolahTests/App/DataFormatVersionBumpTests.swift` still tracks `current` symbolically.
