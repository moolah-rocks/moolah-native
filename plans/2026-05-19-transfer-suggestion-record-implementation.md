...
- **Parameter-label rename + ALL call sites (closes a Phase-4-review gap — REQUIRED).** Task 10 renamed the `TransferDetectionCoordinator` init param `dismissedPairs:` → `suggestions:`, but `TransactionStore.init` still declares `dismissedTransferPairs: (any DismissedTransferPairRepository)? = nil` and passes it as `dismissedPairs:`. Until this is fixed the app/benchmark targets do not build. In this task also:
  ...
  - **Total sweep (no file left behind — nothing deferred).** After the listed edits, run `git -C <worktree> grep -rln "DismissedTransferPair\|dismissedTransferPairs\|\bdismissedPairs\b\|\.transferSuggestion\b" -- Features/ App/ MoolahBenchmarks/ MoolahTests/ UITestSupport/ MoolahUITests_macOS/` and fix EVERY hit (production, benchmark, **all of MoolahTests/**, UI-test support) — not only the files this plan enumerated. Any `MoolahTests/` test still constructing `TransferDetectionCoordinator(...,dismissedPairs:)`, reading `transaction.transferSuggestion`, or referencing the deleted type must be migrated to the record model (`suggestions: backend.transferSuggestions`; assert via `backend.transferSuggestions`) so `just test` links AND passes at Task 13. (`MoolahTests/Features/Sync/SyncedAccountStoreTransferDetectionTests.swift` is reconciled in Task 11; if any other test file surfaces here, fix it now — do not defer.) Expected final result: ZERO hits across the entire grep. The only legitimate remaining occurrences anywhere are in `plans/*.md` (historical prose) — nowhere in code.
