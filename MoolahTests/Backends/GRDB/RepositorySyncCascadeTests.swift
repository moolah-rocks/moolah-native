...
struct RepositorySyncCascadeTests {
  ...
  /// null `transaction_leg.account_id` references — replacing what the v4
  /// FK SET NULL did before v5 dropped the FKs.
  @Test
  func accountSyncDeleteNullsLegs() async throws {
    ...
    try await database.write { database in
      try database.execute(
        sql: """
          ...
          """,
        arguments: [accountId, txId, legId, txId, accountId])
    }
