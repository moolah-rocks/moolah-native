...
/// Direct unit tests for `GRDBAnalysisRepository.assembleUncategorisedBalances`
/// — the SQL-aggregation post-processor that converts each
/// `(day, instrument)` row to the target instrument and accumulates a
/// single running total.
///
/// Mirrors `GRDBCategoryBalancesAssembleTests` 1:1 (see its file header for
/// the full rationale): these tests drive the static helper directly with
/// an injected throwing conversion service (`FakeConversionService.perCall`
/// from `MoolahTests/Support/`) so the per-row error contract required by
/// `INSTRUMENT_CONVERSION_GUIDE.md` Rule 11 is captured by a unit test (no
/// GRDB stack needed). A future refactor that collapses the per-row
/// `do/catch` shape — e.g. wrapping the entire loop in an outer catch and
/// logging once — will trip the failure-count and rethrow assertions
/// below.
@Suite("GRDBAnalysisRepository.assembleUncategorisedBalances — Rule 11 contract")
struct GRDBUncategorisedBalancesAssembleTests {
  ...
  private func makeAggregation()
    -> GRDBAnalysisRepository.UncategorisedBalancesAggregation
  {
    let usd = "USD"
    let rows: [GRDBAnalysisRepository.UncategorisedBalancesRow] = [
      .init(day: "2025-01-15", instrumentId: usd, qty: -100),
      .init(day: "2025-01-16", instrumentId: usd, qty: -200),
      .init(day: "2025-01-17", instrumentId: usd, qty: -300),
    ]
  ...
  func handleConversionFailureFiresPerRow() async throws {
    ...
    let handlers = GRDBAnalysisRepository.UncategorisedBalancesHandlers(
    ...
    await #expect(throws: CallbackError.self) {
      _ = try await GRDBAnalysisRepository.assembleUncategorisedBalances(
