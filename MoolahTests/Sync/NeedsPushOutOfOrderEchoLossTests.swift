...
///
/// The #1084 fix rests on one load-bearing assumption, stated verbatim in
...
///      delivery — is applied LAST. The row is clean, so it takes the
///      normal upsert path and clobbers V_update back to V_create. LOST.
///
...
struct NeedsPushOutOfOrderEchoLossTests {
  ...
  func legUpdateSurvivesOutOfOrderEcho() async throws {
    ...
    // the version that will echo back stale.
    let vCreateEcho = vCreateRow.toCKRecord(in: Self.zoneID)
  ...
  /// blob present, incoming fields differ" — they are indistinguishable
  /// without an orderable change tag, which CloudKit does not provide.
  /// This test must stay green; any fix that rejects the stale echo by
  /// "clean + has-cached-fields + fields-differ" also drops this.
  ...
  func syncedCleanRowAppliesGenuineRemoteChange() async throws {
    ...
    _ = try repo.setEncodedSystemFieldsSync(
      id: id, data: localRow.toCKRecord(in: Self.zoneID).encodedSystemFields)
    // needs_push stays 0 (clean — fully round-tripped).

    // A peer device's genuine update: quantity 500.
