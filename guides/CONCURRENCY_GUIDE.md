...
**Carve-out 5 — `ScriptCommandBox` (`private` to `NSScriptCommand+ScriptHelpers.swift`, one-way main-thread hand-off).** `Automation/AppleScript/Commands/NSScriptCommand+ScriptHelpers.swift` defines a `private final class ScriptCommandBox: @unchecked Sendable` holding a single `let command: NSScriptCommand`. AppleScript handlers run inside `performDefaultImplementation` — an Objective-C override that the compiler treats as **nonisolated** (an override cannot add `@MainActor` over a nonisolated superclass method), so `self` is task-isolated. The handler's real work is async and `@MainActor`, so `runBlockingWithError` uses Cocoa's documented async-command pattern: `suspendExecution()`, run the work in a `@MainActor` `Task`, then queue `resumeExecution(withResult:)` onto a fresh main run-loop turn. The extra run-loop turn is required because AppKit may synchronously re-enter its KVC scripting bridge while encoding scriptable results; doing that from inside the Swift task can corrupt the active executor on macOS 27. The task and queued callback must hold the command to resume it. The constraint is **one-way ownership transfer only — no concurrent readers**: Cocoa's scripting contract guarantees a suspended command is dormant until resumed, and both stages touch it only on the main thread. Swift's region-isolation pass can't model that contract (capturing `self` directly trips `sending 'self' risks causing data races`), and the only alternative the compiler would accept — a `DispatchSemaphore` to block until the async work completes — is the forbidden anti-pattern that parks a cooperative-pool thread. The `@unchecked` waives only the structural `Sendable` check on a non-`Sendable` Cocoa type; the box introduces no shared mutable state. The justification is repeated as a doc-comment on the class itself. Anything that would let a second thread observe `ScriptCommandBox.command` invalidates this carve-out — keep the box internal to `runBlockingWithError`.
